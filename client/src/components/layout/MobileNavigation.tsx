...
export default function MobileNavigation() {
  ...
  const mobileNavItems = navigationTabs.filter(tab => ["home", "videos", "books"].includes(tab.id));
      ...
      <div className="flex justify-around items-center h-16 px-2">
        {mobileNavItems.map(tab => (
          <Link key={tab.id} href={tab.path}>
            <div className={`flex flex-col items-center justify-center cursor-pointer w-20 h-full ${
              location === tab.path || 
