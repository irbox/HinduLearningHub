    ...
    <header className="sticky top-0 z-50 bg-background border-b border-border shadow-sm">
      <div className="container mx-auto px-4 py-3">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex-1 flex items-center gap-4">
            <form
              ...
              className="flex-1 mx-2 md:mx-4 max-w-xl rounded-full bg-muted px-2 md:px-3 py-1 border border-border hidden sm:flex"
            >
              <Input
                type="text"
              ...
              <div className="flex items-center gap-1">
                ...
                <Button
                  ...
                  variant="ghost"
                  size="icon"
                  className="text-primary h-8 w-8 hidden md:flex"
                >
            ...
            <Sheet open={isSearchOpen} onOpenChange={setIsSearchOpen}>
              <SheetTrigger asChild>
                <Button
                  variant="ghost"
                  size="icon"
                  className="sm:hidden text-primary h-8 w-8"
                >
        ...
        <nav className="hidden md:flex items-center">
          {navigationTabs
            .filter((tab) => ["home", "videos", "books"].includes(tab.id))
            .map((tab) => (
              <Link key={tab.id} href={tab.path}>
                <span
                  className={`whitespace-nowrap px-3 py-1 text-sm font-medium cursor-pointer flex items-center gap-1 ${
                    location === tab.path ||
