...
import { Search, Filter, Sparkles, RefreshCw, BookOpen, VideoIcon } from "lucide-react";
...
export default function SearchPage() {
  ...
  const channels = [
    { id: "govardhan-math", name: "Govardhan Math, Puri", value: "Govardhan Math, Puri" },
  ...
  const { data: searchResults, isLoading, error } = useQuery<{videos: Video[], books: Book[]}>({
            ...
            <div className="space-y-6">
              {allResults.map((result, index) => (
                <div key={`${result.type}-${index}`}>
                  ...
                  {index < allResults.length - 1 && <Separator className="my-4" />}
                </div>
              ))}
            </div>
          )}
