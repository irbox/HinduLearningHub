...
import { 
  ChevronLeft, Download, Share2, Bookmark, User, FileText, 
  BookText, AudioLines, GraduationCap, Search, Filter, CheckCircle, Eye 
} from "lucide-react";
  ...
  if (!book && id) {
    return (
      <div className="container mx-auto px-4 py-12 text-center">
        ...
        <p className="mb-6">The book you are looking for doesn't exist or has been removed</p>
        <Link href="/books">
        ...
        <div className="lg:w-2/3">
          <div className="flex flex-col md:flex-row gap-6 mb-8">
            ...
            <div className="md:w-2/3">
              <h1 className="text-2xl font-semibold text-primary mb-2">{book?.title}</h1>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                {book?.format && (
                  <div>
                    <span className="text-muted-foreground">Format:</span> {book.format}
                  </div>
                )}
              </div>
            </div>
                  ...
                  <div className="space-y-2">
                    {book.tableOfContents.map((chapter, index) => (
                      <div key={index} className="flex justify-between">
                        ...
                        <span className="text-muted-foreground">p. {chapter.page}</span>
                      </div>
                    ))}
                  </div>
                ) : (
                      ...
                      <div key={index} className="pb-4 border-b border-gray-700 last:border-0 last:pb-0">
                        <div className="flex items-center mb-2">
                          <div className="h-8 w-8 bg-primary/20 rounded-full flex items-center justify-center mr-2">
                            <span className="text-sm">{review.author.charAt(0)}</span>
                          </div>
                          <div>
              ...
              <Link key={book.id} href={`/books/${book.id}`}>
                <div className="bg-surface p-3 rounded-lg hover:bg-surface/60 transition cursor-pointer">
                  <div className="flex gap-3">
                    ...
                    />
                    <div>
                      <h3 className="text-sm font-medium text-primary">{book.title}</h3>
